<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($errName) && empty($errPass) && empty($errGender) && empty($errEmail) && empty($errPhone)){
            ...
            $sqlCheck = "SELECT * FROM users WHERE email = '{$email}' OR phone = '{$phone}'";
            ...
            if(mysqli_num_rows($resultCheck) > 0){
                ...
                $sql = "INSERT INTO users (name, password, gender, email, country_code, phone) VALUES ('{$name}', '{$password}', '{$gender}', '{$email}', '{$country_code}', '{$phone}')";
